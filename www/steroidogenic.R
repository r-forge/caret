...
rfeProfiles <- rbind( nbRFE$results[, c("Variables", "ROC", "Model")],
                      rfRFE$results[, c("Variables", "ROC", "Model")],
                      lrRFE$results[, c("Variables", "ROC", "Model")],
                     svmRFE$results[, c("Variables", "ROC", "Model")])
...
## Merge compound number with RF holdout data
meanPred <- ddply(rfHoldouts, .(rowIndex, obs), function(x) c(Average = mean(x$toxic)))
meanPred$Compound <- rownames(imputed)[meanPred$rowIndex]

prob_and_thle <- merge(meanPred, thle)

xyplot(Average ~ THLE, data = prob_and_thle,
       ylab = "Average Probability of Toxicity",
       xlab = "Cytotoxicity  IC50 (uM)",
       cex = 1.1,
