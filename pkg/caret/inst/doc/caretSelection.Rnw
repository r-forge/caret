...
@ 
Of the \Sexpr{ncol(x)} predictors, there are \Sexpr{ncol(x)-5} pure noise variables: 5 are uniform on [0, 1] and \Sexpr{p} are random univariate standard normals.
...
The simulation will fit models with subset sizes of \Sexpr{paste(sort(subsets, decreasing = TRUE), collapse = ", ")}. A linear model and random forests will be used.
...
pdf("tolerance.pdf", width = 5, height = 5)
...
with(example, plot(Variables, RMSE))
