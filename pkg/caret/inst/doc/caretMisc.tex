...
The \texttt{caret} package (short for {\bf{\color{blue}{c}}}lassification {\bf{\color{blue}{a}}}nd {\bf{\color{blue}{re}}}gression {\bf{\color{blue}{t}}}raining) contains functions to streamline the model training process for complex regression and classification problems. The package utilizes a number of R packages but tries not to load them all at package start-up\footnote{By adding formal package dependencies, the package startup time can be greatly decreased}. The package ``suggests'' field includes: \texttt{ada}, \texttt{affy}, \texttt{Boruta}, \texttt{bst}, \texttt{caTools}, \texttt{class}, \texttt{Cubist}, \texttt{doMC}, \texttt{e1071,}, \texttt{earth ($\ge$ 2.2-3)}, \texttt{elasticnet}, \texttt{ellipse}, \texttt{fastICA}, \texttt{foba}, \texttt{gam}, \texttt{GAMens ($\ge$ 1.1.1),}, \texttt{gbm}, \texttt{glmnet}, \texttt{gpls}, \texttt{grid}, \texttt{hda}, \texttt{HDclassif}, \texttt{Hmisc}, \texttt{ipred}, \texttt{kernlab}, \texttt{klaR}, \texttt{lars}, \texttt{leaps}, \texttt{LogicForest,}, \texttt{logicFS}, \texttt{LogicReg}, \texttt{MASS}, \texttt{mboost}, \texttt{mda}, \texttt{mgcv}, \texttt{mlbench}, \texttt{neuralnet}, \texttt{nnet}, \texttt{nodeHarvest}, \texttt{pamr,}, \texttt{partDSA}, \texttt{party ($\ge$ 0.9-99992)}, \texttt{penalized}, \texttt{pls}, \texttt{proxy}, \texttt{qrnn}, \texttt{quantregForest}, \texttt{randomForest}, \texttt{RANN}, \texttt{rda}, \texttt{relaxo}, \texttt{rocc}, \texttt{rpart}, \texttt{rrcov}, \texttt{RWeka ($\ge$ 0.4-1)}, \texttt{sda}, \texttt{SDDA}, \texttt{sparseLDA ($\ge$ 0.1-1),}, \texttt{spls}, \texttt{stepPlr}, \texttt{superpc}, \texttt{vbmp}. \texttt{caret} loads packages as needed and assumes that they are installed. Install \texttt{caret} using 
\begin{Verbatim}
...
For example, the \texttt{etitanic} data set in the \texttt{earth} includes two factors: \texttt{pclass} with levels 1st, 2nd, 3rd and \texttt{sex} with levels female, male. The base R function \texttt{model.matrix} would generate the following variables:
\begin{Schunk}
\begin{Sinput}
> library(earth)
> data(etitanic)
...
\begin{itemize}\raggedright
  ...
  \item Other packages: caret~4.99, class~7.3-3, cluster~1.14.0,
    ...
    ellipse~0.3-5, foreach~1.3.2, gbm~1.6-3.1, ipred~0.8-11,
    iterators~1.0.5, kernlab~0.9-12, klaR~0.6-5, lattice~0.19-31,
    leaps~2.9, MASS~7.3-14, mlbench~2.1-0, multicore~0.1-5, nnet~7.3-1,
    plotmo~1.2-6, plotrix~3.2-2, pls~2.1-0, plyr~1.5.2, proxy~0.4-6,
    randomForest~4.6-2, reshape~0.8.4, rpart~3.1-50, survival~2.36-9
  \item Loaded via a namespace (and not attached): tools~2.13.1
\end{itemize}
\section{References}
