...
{
  if("2class" %in% check)
    if(is.factor(y) && length(levels(y)) > 2) stop("this model is constrained to 2 classes")
  checkZV <- function(x) length(unique(x)) == 1
  if("zv"  %in% check & !is.null(x))
  {
    zv <- unlist(lapply(x, checkZV))
    if(any(zv)) stop("some predictors have a single unique value")
  }
  if("zvPerClass"  %in% check & !is.null(x))
  {
    x$.classCol <- y
    zv <- ddply(x, ".classCol", function(x) any(unlist(lapply(x[, names(x) != ".classCol", drop = FALSE], checkZV))))
    if(any(zv$V1)) stop("some predictors have a single unique value within a class")
  }
  invisible(NULL)
}
